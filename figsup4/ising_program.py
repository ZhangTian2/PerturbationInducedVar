...
class IsingModel2D:
    def __init__(self, size=50, h0=0.0, J=5.0, device='cuda' if torch.cuda.is_available() else 'cpu'):
        """
        初始化2D Ising模型
        ...
        self.spins = torch.ones((size, size), device=device)  # 全部初始化为+1
...
def simulate_with_noise_analysis(
    size=50, h0=0.0, J=1.0, ha_std=0.001,
